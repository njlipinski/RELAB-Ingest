...
def get_material_class(id, source, genType1, gen_type_2, type1, subtype):
...
def load_sample_data():
    ...
    for sample in range(1, sample_cat.nrows):
        ...
        m_class = get_material_class(sample_id, source, gen_type_1, gen_type_2, type_1, sub_type)
...
if __name__ == "__main__":
    ...
    for spectra in range(1, spectra_cat.nrows):
        ...
        source = spectra_cat.cell_value(rowx=spectra, colx=8)
        detect = spectra_cat.cell_value(rowx=spectra, colx=9)
        
        date_added = xlrd.xldate_as_datetime(date_added, 1).date().isoformat()
        view_geo = f'{source}° / {detect}°' if (source!="NA" and detect!="NA") else ""
        ...
        header = [
            ...
            ["Grain Size", f'{spectra_sample_data["min_grain_size"]} - {spectra_sample_data["max_grain_size"]}'],
            ["Locality", ""],
            ...
            ["Material Class", spectra_sample_data["MaterialClass"]],
        ...
        try:
            ...
            data = [
                [
                    ...
                    str(float( Decimal(line.split("\t")[1]) * MICRON_TO_NM ))
                ]
                for line in lines
            ]
        except:
        ...
        output_dest = f'output/{pi_initials}_{sub_folder_name}_{spectra_id.lower()}.csv'
